    ...
    public partial class SalesInvoice : Form
    {
        ...
        private bool ValidateCustomerDetail()
        {
            if (string.IsNullOrWhiteSpace(txtCustomerName.Text) ||
                ...
                string.IsNullOrWhiteSpace(txtCustomerMobileNo.Text))
            {
        ...
        void opp()
        {
            try
            {
                //If customer detail Search through MobileNumber and CustomerName and then retrive
                using (SqlCommand cmd = new SqlCommand("USP_usermaster", con))
                {
                    ...
                    if (cutId == 0)
                    {
                        //If customer detail is not found then insert
                        SqlCommand cmd1 = new SqlCommand("USP_usermaster", con);
                        con.Open();
                        ...
                        //cmd1.Parameters.AddWithValue("@Customer_State_id", Convert.ToInt32(cmbState.SelectedIndex+1));
                        //cmd1.Parameters.AddWithValue("@Customer_State", cmbState.Text.Trim());
            ...
            {
                MessageBox.Show("Data Searching or data insertion in Error" + ex.Message);
            }
        }
